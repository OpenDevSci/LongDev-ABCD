...
# Generalized Linear Mixed Effects Models {.unnumbered}

## Overview

<p>

This example assesses xxxxxxxx.
...
selected_vars <- c("src_subject_id", "eventname", "nihtbx_totalcomp_fc", "anthroweightcalc", "tlfb_alc_sip_l")
...
df <- merged_data %>%
  ...
  mutate(
    ...
    tlfb_alc_sip_l = as.factor(tlfb_alc_sip_l)
  ) %>%
...
compute_descriptives <- function(data, event_name) {
  ...
  tlfb_alc_sip_l_desc <- paste0(table(data$tlfb_alc_sip_l), " (", round(100 * prop.table(table(data$tlfb_alc_sip_l)), 1), "%)")
  ...
  desc_df <- data.frame(
    Variable = c("Sex - Female", "Sex - Male or other", "Race - Asian", "Race - Black", "Race - Other/Mixed", "Race - White", "tlfb_alc_sip_l - No", "tlfb_alc_sip_l - Yes", "Age", "Weight"),
    Value = c(sex_desc, tlfb_alc_sip_l_desc, race_desc, age_desc, weight_desc)
  )
...
```{r}
model <- glmer(tlfb_alc_sip_l ~ eventname + (1 | src_subject_id), data = df, family = binomial(link = "logit"))
```
